    ...
    private function readColInfo()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            for ($i = $fc; $i <= $lc; ++$i) {
                ...
                $offset = empty($this->mapCellXfIndex[$xfIndex])
                    ? (!empty($this->mapCellXfIndex[$xfIndex+1])
                        ? $this->mapCellXfIndex[$xfIndex+1]
                        : !empty($this->mapCellXfIndex[$xfIndex-1])
                            ? $this->mapCellXfIndex[$xfIndex-1]
                            : false
                    )
                    : $this->mapCellXfIndex[$xfIndex];
                if ($offset == false) {
                    break;
                }
                ...
                $this->phpSheet->getColumnDimensionByColumn($i)->setXfIndex($offset);
            }
        }
    }
